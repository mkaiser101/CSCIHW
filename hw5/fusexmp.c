...
static void xmp_realpath(char rpath[Maxpath], const char *path)
{
    strcpy(rpath, XMP_DATA->dir);
    strncat(rpath, path, Maxpath); // ridiculously long paths will break here
}
...
static int xmp_encrypt (char template[], char* rpath)
{
  ...
  if(!do_crypt(inFile, outFile, ENCRYPT, XMP_DATA->key)){
...
static int xmp_decrypt(char template[], char* rpath)
{
  ...
  if(!do_crypt(inFile, outFile, DECRYPT, XMP_DATA->key)){
    fprintf(stderr, "do_crypt failed\n");
  }
...
int main(int argc, char *argv[])
{
	...
	xmp_data->dir = realpath(argv[argc-2], NULL);
